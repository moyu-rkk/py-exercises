...
#so you can't put an int in it while creating a lits?string? for math calculating looks like it works
...
def current_deck(player_deck, player_score, dealer_deck):
    current_deck = f"  Your cards: {player_deck}, current score: {player_score}\n  Computer's first card: {dealer_deck[0]}\n"
    return current_deck

def final_deck(player_deck, player_score, dealer_deck, dealer_score):
    final_deck = f"  Your final hand: {player_deck}, final score: {player_score}\n  Computer's final hand: {dealer_deck}, final score: {dealer_score}\n"
    return final_deck
    ...
    while flag:
        ...
        if compare == 0 or compare == 1 or compare == 2 or compare == 3 or compare == 4 or compare == 6:
            ...
            while draw == 'y':
              ...
              #or the continue go on executing the nested loop 
              #thats's the reason when you choose starting a new game it doesn't create new decks instead it keeps on going with the old decks
